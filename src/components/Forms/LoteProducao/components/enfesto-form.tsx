  ...
  useEffect(() => {
    if (coresParaSelecionar.length === 1 && !selectedCorId) {
      ...
      onChange(index, {
        ...enfesto,
        corId: corUnica.id,
        rolosProducao: corUnica.rolos && corUnica.rolos.length > 0 ? [
          {
            estoqueRoloId: corUnica.rolos[0].id,
            pesoReservado: corUnica.rolos[0].pesoAtualKg,
          },
